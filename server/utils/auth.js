...
module.exports = {
  ...
  authMiddleware: function ({ req }) {
    ...
    try {
      // else if a token is found, verify it
      const { authenticatedPerson } = jwt.verify(token, secret, {
        maxAge: expiration,
      }); // Extract the authenticatedPerson from the token payload (contains user data like ID and fullName)
  ...
  signToken: function ({ email, fullName, _id }) {
    // extracts only the fullName email id from the user object passed to this function from the addUser Resolver
    const payload = { email, fullName, _id }; // Create a payload with the user's email, fullName, and ID
